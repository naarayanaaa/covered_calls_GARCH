...
import sys
# Ensure we can import from io directory without conflict
...
def load_earnings_dates(ticker, base_dir):
    ...
    Looks for parquet/csv files in data/{ticker}/**/events_earnings
    """
    # Simple search pattern
    # Assuming standard structure data/TICKER/YYYYMMDD.../events_earnings.parquet
    search_path = os.path.join("data", ticker, "**", "events_earnings.*")
    files = glob.glob(search_path, recursive=True)

    if not files:
        # Try args.out_dir if specified differently in main logic (though main logic usually uses ./output)
    ...
    for exp_date, group in opts.groupby('expiration'):
        ...
        if not earnings_df.empty:
            ...
            if not future_earnings.empty:
                ...
                # If earnings falls within the option's life (with 1 day buffer)
        ...
        # 1. Get GARCH annualized vol
        # garch.res.conditional_volatility is daily sigma
        last_garch_sigma = garch.res.conditional_volatility.iloc[-1]
        garch_vol_annual = last_garch_sigma * np.sqrt(252)

        # 2. Get Market ATM IV
        # Find call strike closest to spot
        ...
        # 3. Panic Switch
        # If Market IV is > 1.5x GARCH, trust the market (something is happening)
        simulation_vol = last_garch_sigma # Default

        if market_iv > 1.5 * garch_vol_annual:
            print(f"  [WARNING] High Volatility Regime: Market IV ({market_iv:.2%}) > 1.5x GARCH ({garch_vol_annual:.2%}).")
            print("  -> Overriding simulation volatility with Market IV for safety.")
            # Convert annual market IV back to daily for simulation
            simulation_vol = market_iv / np.sqrt(252)

        # Simulate Paths using calibrated vol
